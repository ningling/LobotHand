...
class LSC_board(object):
	...
	def __init__(self,baudrate=9600, serialport="/dev/ttyS0"):
	 	...
	 	logger.debug("Setting serial communication at Baud rate:"+str(self.baudRate)+"\nSerial Port:"+self.serialPort)
	...
	def SingleServoMove(self,speed=1000, servoData=[]):
		if len(servoData)!=2:
			logger.debug("Single Servo Move data should have two elements: [Servo ID,Angle]")
			return -2
		if speed<SPEED_LOWER_BOUND:
			logger.debug("Speed setting too low! Using lower bound instead")
			actionSpeed=SPEED_LOWER_BOUND
		if speed>SPEED_UPPER_BOUND:
			...
			actionSpeed=SPEED_UPPER_BOUND
		actionSpeed=speed
		cmdString=FRAME_HEAD+chr(0x8)+chr(CMD_SERVO_MOVE)+chr(0x01)+chr(actionSpeed&0xFF)+chr(actionSpeed>>8)
		cmdString+=chr(servoData[0])+chr(servoData[1]&0xFF)+chr(servoData[1]>>8)
		self.serialComm.write(cmdString)
		return 1
